...
**🚨 PROD patch → NO BA test; self-verify + close (2026-09-07, per みや)**: after infra runs a PROD data patch, I verify the outcome MYSELF via the DB (the before/after SELECTs I hold, [[feedback_never_hand_miya_a_query]]) and then CLOSE — never park it on "BA retest" / "ask BA to confirm". BA-testing is for CODE fixes deployed to int-env/staging, not PROD data patches. The DB verification IS the proof. Close = run `quest/archive-quest.js <id>` (accepts `ADHOC-*` since 2026-09-07) — folder→Archive (script travels inside), active.txt block→active-archive.
