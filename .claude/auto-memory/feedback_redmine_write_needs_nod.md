...
> **2026-09-04 (#275847) — now DETERMINISTIC**: `domain/redmine-write-gate` (PreToolUse on Bash|PowerShell) BLOCKS any Redmine write unless the LAST user message is an explicit approval (`post it` · `post now` · `Yes, post …` · `[redmine-post-ok]`). A wording instruction ("start with Salam …") is NOT approval — that exact shape posted a note miya wanted to review, and Redmine journals cannot be edited via the API (404). Show the note fenced, wait for the phrase.
