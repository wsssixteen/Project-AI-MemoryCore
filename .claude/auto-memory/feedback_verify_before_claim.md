...
- **Config/reference-value claims need the config-table query, not a code read** (added 2026-09-22, QA-280540): A root-cause or fix-shape claim that decides a branch on a config / reference value cites the config-table query result on every env in play (`<table.column> = <value> on <env>`), never a code read alone — see [[config-table-before-code]].
