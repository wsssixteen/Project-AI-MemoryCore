...
---
**v2 update (2026-09-20):** All 69 class recordings transcribed → `projects/learning-projects/active/arabic/source/transcripts/NN.ms.md` (view-only 48 recovered via [[drive-viewonly-download]]). Library built at `.../active/arabic/library/` (README, recordings, PLAN-v2, run-state, scripts, classes/01). **Plan v2 APPROVED** by みや: evolve /arabic into a syllabus-driven study system (5 phases) — build the frozen `data/syllabus.json`+`words.json` (root+harakat+form-tables), then form/sentence/topic revision modes, then Drive-sync. Governed by [[mechanical-deterministic]] (build-once-verify-freeze / use=deterministic-script). Resume: `library/HANDOFF-arabic-v2.md` → Phase 1.
