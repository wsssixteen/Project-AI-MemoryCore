...
- [feedback_uat_fat_environments.md](feedback_uat_fat_environments.md) — UAT (mlkuat / et_main_uat) is local testing default; FAT (mlkfat / etprdmlk / et_main) is for simulation only; AWAM tickets test on UAT (mkit / et_main_mlit) regardless of where BA reported them; flowable alter page shifts test apps between tugasan steps
