...
**Current**: v1.51 (2026-06-20 — §8 Minimal-diff discipline bullet + comment-strip-at-Phase-1 (`prepare-commit-trigger` Step 2.6) + `convention-check-gate` jsf-kind/in-file-first (QA-261986); per みや). **Prev**: v1.50 (2026-06-19 — §10 AWAM baseline → `mlk/stag-env` + Scout step-0.5 existing-fix probe; per みや QA-266215/266249). **Recent 2**: v1.49 (decomposed footer to `meta/claude-md-changelog.md` per みや) · v1.48 — Git Scout + DB binding + §11 collapse (`c277332`). **Full history**: see `meta/claude-md-changelog.md` (all v1.29 → current with rationale per version + the version-bump discipline rule).
