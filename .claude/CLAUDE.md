...
**Current**: v1.52 (2026-06-20 — compulsory Phase-0 git-state check: `ticket-gate.js` Row 0 + Quest-Prep-Verification row + Phase-1 remote-probe in `prepare-commit-trigger` + slip-log `stage`/git sub-categories; per みや git-check research). **Prev**: v1.51 (2026-06-20 — §8 Minimal-diff + comment-strip-at-Phase-1 + `convention-check-gate` jsf/in-file + ticket-criteria-gate Power). **Recent 2**: v1.50 (2026-06-19 — AWAM baseline → `mlk/stag-env`) · v1.49 (decomposed footer to `meta/claude-md-changelog.md`). **Full history**: see `meta/claude-md-changelog.md` (all v1.29 → current with rationale per version + the version-bump discipline rule).
