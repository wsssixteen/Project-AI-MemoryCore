...
- **Branch / pull discipline**: at Phase 0 start, `git status && git branch --show-current` on relevant repos. **Baseline is per-repo: `mlk/master` (etanah-pelupusan) · `mlk/stag-env` (etanah-awam — the staging integration branch where `[STAGING] AWAM` tickets are tested + shared fixes land; changed 2026-06-19 per みや, was `mlk/release/fat`).** If not on the repo's baseline, do the standard sequence (stash → checkout baseline → pull --ff-only → stash pop). At Phase 1 close, pull before branching (see Phase 1 Closure section below).
...
**Current**: v1.53 (2026-06-27 — §2 Structured-Separated Problem/Cause/Fix explanation shape + commit-conventions ESOKONGAN→`mlk/esokongan/<num>` branch mapping + general `mlk/<tracker>/<num>` derive-rule; per みや QA-267382). **Prev**: v1.52 (2026-06-20 — compulsory Phase-0 git-state check: `ticket-gate.js` Row 0 + Quest-Prep-Verification row + Phase-1 remote-probe in `prepare-commit-trigger` + slip-log `stage`/git sub-categories). **Recent**: v1.51 (2026-06-20 — §8 Minimal-diff + comment-strip-at-Phase-1 + `convention-check-gate` jsf/in-file + ticket-criteria-gate Power). **Recent 2**: v1.50 (2026-06-19 — AWAM baseline → `mlk/stag-env`) · v1.49 (decomposed footer to `meta/claude-md-changelog.md`). **Full history**: see `meta/claude-md-changelog.md` (all v1.29 → current with rationale per version + the version-bump discipline rule).
