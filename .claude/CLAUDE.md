...
The ordered `pull → checkout -b → stage → commit → push → /verify → checkout mlk/master → pull --ff-only → update active.txt` close-out sequence — see `quest/quest-protocol.md` → Phase 1 close-out + the **Commit + Push hard rule** (migrated there 2026-05-22). Runs ONLY after `local_test_confirmed=true`; the `mlk/qa/*` branch is created at Commit prep, never at Apply. Durable fix in flight = the `/branch-and-push` script (todo.md Q2).

**🚨 UPDATE-ACTIVE.TXT-AT-PHASE-1-CLOSE — HARD RULE (added 2026-06-02 per みや, QA-260508 close).** Phase 1 close is NOT complete at `pull --ff-only`. The FINAL step before declaring done is updating the quest's `active.txt` block to reflect post-push state, with these fields set: `phase=1` · `status=active` · `current_phase=Push` · `branch=mlk/qa/<num>` · `commit=<sha>` · `pushed=<YYYY-MM-DD>` · `local_test_confirmed=true` · `note=Phase 1 closed <date> — branch mlk/qa/<num> pushed; awaiting BA verification on <env>`. **Why**: without this, the next session's boot-time `open-quest-surfacer.js` + briefing has stale phase=0 status for a quest that's actually at phase=1 Push-awaiting-BA, and the briefing falsely surfaces the quest as "needs Apply" when it actually needs only BA-verification. **How to apply**: at every Phase 1 close emit, the visible ✓-line is `branch=mlk/qa/<num> pushed ✓ · returned to mlk/master ✓ · pulled --ff-only ✓ · active.txt updated ✓`. **Banned**: declaring Phase 1 closed without the active.txt update step visible. Cross-ref: pairs with the **return-to-master-after-push** rule below + the **branch-at-Apply-is-banned** rule.

**🚨 RETURN-TO-MASTER-AFTER-PUSH — HARD RULE (added 2026-06-02 per みや, QA-246923 → QA-260508 slip).** Phase 1 close is NOT complete at `push`. The final two steps are `git checkout mlk/master && git pull --ff-only` — the tree must end on `mlk/master` synced with origin. **Why**: leaving the tree on `mlk/qa/<num>` after push means the NEXT quest's Apply edits land on a stale qa branch as their base — invisible cross-contamination between tickets. みや caught this 2026-06-02 when QA-260508 Apply context showed `current_branch = mlk/qa/246923` instead of `mlk/master`. **How to apply**: at every Phase 1 close emit, the visible ✓-line is `branch=mlk/qa/<num> pushed ✓ · returned to mlk/master ✓ · pulled --ff-only ✓`. **Banned**: declaring Phase 1 closed without the return-to-master step visible. Cross-ref: pairs with the **branch-at-Apply-is-banned** rule above (Apply lands on the current branch, which post-Phase-1 must be mlk/master).

**🚨 BRANCH-AT-APPLY-IS-BANNED — HARD RULE (added 2026-06-02 per みや, QA-260508 slip).** At Apply time, the working tree is on whatever branch it's already on (typically `mlk/master` after a prior Phase 1 close, OR the existing rework branch if continuing a held quest). **DO NOT create `mlk/qa/<num>` at Apply.** The branch creation is part of the **Commit prep sequence at Phase 1 close-out**, which fires ONLY after みや confirms local-test success (`local_test_confirmed=true`). Apply edits land on the current branch; commits and the new `mlk/qa/<num>` branch come LATER. **Banned**: emitting "Branch will be `mlk/qa/<num>` from `mlk/master`" at the Apply-readiness gate · running `git checkout -b mlk/qa/<num>` before tests pass. **Why** (QA-260508 2026-06-02): at the Apply-readiness emit I proposed `Branch will be mlk/qa/260508 from mlk/master` as part of the "ready to Apply" sequence — みや caught it: branching at Apply means a fix that fails local testing leaves an orphan branch + dirty state, OR forces a destructive branch-delete rollback. Branching belongs to the moment-of-commit (after green tests), not the moment-of-edit. **How to apply at Apply emit**: state "Apply will edit on current branch (`<branch-name>`); branch + commit are deferred to Phase 1 close after your local-test confirmation." Cross-ref: pairs with the **work-repo cleanup** rule + the **stop-at-stage gate** at Phase 1 close.
...
**Current**: v1.49 (decomposed footer to `meta/claude-md-changelog.md` per みや — was 5k+ tokens loading every session). **Recent 2**: v1.48 — Git Scout + DB binding + §11 collapse (`c277332`). v1.47 — audit-driven hardening of 2 slipped rules (`782ffd9`). **Full history**: see `meta/claude-md-changelog.md` (all v1.29 → current with rationale per version + the version-bump discipline rule).
