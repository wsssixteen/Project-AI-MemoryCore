...
**🚨 FORCED PHASE-EMIT GATES — the loop only works when each phase produces a VISIBLE emit before the next (HARD RULE, added 2026-05-31 per みや, QA-259702).** The decomposition/trim kept this *arrow text* but lost the *forced emits* — so a session can "run the quest skill" yet freelance straight from a glance to an Edit, skipping Recon + Rubric. That is exactly what failed QA-259702 (built a new method instead of grepping the file for its own idiom). **The rule — during ANY quest, these emits are MANDATORY, in order, and an Edit to code/template/config is BANNED until they exist in THIS session:**
   ...
   - **📐 Predicate Diagram** (replaces v1.x Predicate Box; renamed 2026-05-31 per みや — plain English over jargon) — **before each code Edit while debugging** (Debug Ritual 1), emit a 3-node ASCII flowchart: Assumption → Evidence → either-matches-or-falsifier. **OR** — when no live Edit is happening (audit / archived-ticket walkthrough / compliance simulation), emit the SAME 3-node shape with `[ASSUMPTION placeholder]` / `[EVIDENCE placeholder]` / `[FALSIFIER placeholder]` labels to prove the shape was understood. Skipping the emit because "no Edit" is BANNED — the shape is mandatory; only the content differs. Overlap with Recon/Rubric is INTENTIONAL — it grounds the pre-Edit moment when stakes are highest. Falsifier branch is the unique part: forces you to name a data shape that would prove the fix wrong, then plant a `QA<num>-PROBE:` logger that would catch it (per Rubric row e + Ritual 6). **🆕 Probe-discharge for ambiguous-VO/method fixes (the B1 refine, 2026-06-14, QA-260508 S1):** when the ASSUMPTION is *"VO/field X holds the data"* or *"method Y is the one that fires"* AND ≥2 candidates exist (two same-type VOs · two handlers · inline-vs-popup), the FALSIFIER MUST be discharged by a **runtime probe** (one-pass dump of ALL candidates) BEFORE the Edit — OR a one-line justification why static reading is conclusive (single binding, or `codegraph_callers`-proven single writer). Static binding-read alone does NOT discharge it. **Why**: QA-260508 — picked `premiumCukaiVO` from the inline-panel binding when the live data sat in the popup's `maklumatPajakanVO.premiumVO`; a probe dump shows which VO holds the data in one pass. Canonical shape:
...
```
   ...
   - **🚨 Per-file sibling-diff EMIT LINE** — **the line IS the rule. Substituting equivalent prose ("checked siblings" / "compared template vs PT" / "scanned the panel") = NOT COMPLIANT.** Before building ANY edited file, emit verbatim ONE line: **`<file:line> ← sibling <working file:line>: attrs ✓ · listener-sig ✓ · VO-instance ✓ · lifecycle ✓`** (or name the specific divergence in place of ✓). Building/deploying without the literal line is BANNED. Archived-ticket / compliance / audit mode does NOT exempt this — if a file was edited in the cycle being walked, emit the line citing what WAS diffed. (Hardened 2026-06-01 after v1.46 haiku audit found this rule slipped 75% with paraphrase-substitution + false-compliance claims.) **🆕 VO-instance disambiguation (the B3 refine, 2026-06-14, QA-260508 S1):** when ≥2 same-type VOs exist on the page, the `VO-instance ✓` token CANNOT be claimed from a static binding-read — emit `VO-instance ⚠ PROBE-REQUIRED (N candidates: <list>)` instead, discharged by a runtime probe-dump or a `codegraph_callers`-proven single-writer (pairs with the Predicate Diagram B1 probe-discharge). **Why**: QA-260508 — `VO-instance ✓` passed on a static read while the chosen `premiumCukaiVO` was all-null at runtime and the popup's `maklumatPajakanVO.premiumVO` held the data.
...
**Current**: v1.50 (QA-260508 B1+B3 §10 refines — B1 Predicate-Diagram probe-discharge for ambiguous-VO/method fixes; B3 sibling-diff `VO-instance ⚠ PROBE-REQUIRED` branch. Companion quest-protocol.md v3.9 = B2/B4/B5; system-architecture.md v1.8). **Recent 2**: v1.49 — decomposed footer to `meta/claude-md-changelog.md` (was 5k+ tokens/session). v1.48 — Git Scout + DB binding + §11 collapse (`c277332`). **Full history**: see `meta/claude-md-changelog.md` (all v1.29 → current with rationale per version + the version-bump discipline rule).
