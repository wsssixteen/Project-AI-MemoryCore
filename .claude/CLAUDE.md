...
- **Test-permohonan discovery — tugasan mismatch is NOT a blocker** (hard rule, 2026-05-17): When みや asks Ruri to find / retrieve / provide a test permohonan ID, Ruri MUST deliver a usable candidate. The absence of an aplikasi sitting at the EXACT target tugasan is **not** a reason to stop and ask みや — みや routinely flowable-alters a permohonan to the needed tugasan. Ruri's job: DB-query the urusan-scoped `umm_aplikasi` candidate set, pick the best fit (correct urusan + valid hakmilik/data chain + relevant panel data present where it matters), and surface it with its current tugasan + an "alter to `<target>`" note. Escalate to みや ONLY when no aplikasi exists for that urusan at all. **Why** (みや 2026-05-17, QA-260302): Ruri repeatedly stopped at "PLTP and PLPS have no test ID — provide them" instead of querying candidates herself. みや: *"I'll alter if there's no specific tugasan... if this is not clear & you kept skipping it & asking me instead, update this rule so it doesn't repeat."* **How to apply**: any "find me a test ID" request → run the urusan-scoped candidate query → provide candidate(s) + current tugasan + "alter to X" note. Banned: "you need to provide an ID for `<urusan>`" when a candidate query was never run. This is the test-data-specific instance of personality.md's Enumerate-then-pursue rule.
  - **Check BOTH FAT and UAT before manufacturing test data — UAT usually already has it (hard sub-rule, 2026-05-17, QA-260302 — みや's "biggest lesson")**: For any test-data hunt — **ESPECIALLY "semua urusan" / multi-urusan tickets** — query the **UAT** DB (`mcp__postgres-mlkuat__query`, schema prefix `et_main_uat.`) as well as **FAT** (`mcp__postgres-mlkfat__query`, schema prefix `et_main.`). UAT carries far more applications parked in varied workflow states, so it usually already has an app sitting at the needed tugasan — **no flowable-alter, no DB-seeding**. **Why**: QA-260302 burned ~15 turns hunting FAT (which had almost no apps in the JPPH-panel render states) → flowable-alters that errored on sub-flow gateways → DB-seeding as the only path; UAT had ready apps across all 5 urusans the whole time. **How to apply**: at the FIRST test-data query of a multi-urusan ticket, run it against UAT too; prefer whichever environment already has an app at the target tugasan. Checking the other environment is far cheaper than altering flowable or seeding the DB. Still verify the chosen app's state — UAT data is not blindly trusted, same rigor as FAT.
  - **Panel-Render Check — 5-step procedure to KNOW (not guess) whether a permohonan + tugasan will render a panel (added 2026-05-17, QA-260302)**: (1) **Screen** — `ind_langkah` + `ind_skrin` → which `jsf_view` the tugasan mounts. (2) **Gate** — read that XHTML → the panel's `rendered="#{...}"` EL. (3) **Flag** — read the Java method that computes that flag → is it true for this urusan + tugasan (deterministic from code — never a guess). (4) **Data** — DB query → does the app hold the rows the panel displays. (5) **Reach — empirical, MANDATORY before naming any test candidate** — query `umm_a_tgsn` task-HISTORY (not just `umm_tgsn_semasa` current-state) for the target tugasan kod, across the whole urusan, in BOTH UAT and FAT. Three outcomes: (a) an app is there now → testable as-is; (b) zero now but ≥1 app of that urusan has *ever* been there (history rows exist) → the tugasan is a live workflow node, flowable-alter is viable; (c) **zero apps ever — current AND history both empty → the tugasan is NOT a reachable workflow node; the flowable-alter page will not offer it. Do NOT recommend it as a flowable-alter candidate. Say so explicitly and surface DB-seed as the only path.** **Why** (QA-260302, 2026-05-17): handed みや flowable-alter targets (KKMB, PYN5A, PYSKP) the alter page did not offer — those tugasans had zero apps ever. The `umm_tgsn_semasa`-only check answers "is it there now" but NOT "can it ever get there"; only the `umm_a_tgsn` history check answers reachability. **Verdict**: panel renders ⟺ steps 1-4 pass AND step 5 is satisfied. **Why**: QA-260302 — Ruri repeatedly handed "confident" test IDs having done only 2-3 of these and guessing the rest; every miss was a skipped step. Run all 5 in full BEFORE naming any candidate; present each step's result as verified / could-not-verify.
  - **Flowable-alter + DB-seed-then-revert is a NORMAL, encouraged tool — not a last resort (added 2026-05-17 by みや)**: build fluency with the alter→test→revert and seed→test→revert cycles; use them freely to manufacture a test state. みや confirmed 2026-05-17 that flowable-alter leaving a history trail is acceptable. Stop treating these as risky exceptions — they are standard test-setup operations.
- **Branch check + pull at Quest Phase 0** (hard rule, 2026-05-04, refined 2026-05-07): Before any code edit on `etanah-pelupusan` or `etanah-awam`, run `git branch --show-current && git status --short` in BOTH repos. **Then ALWAYS run `git pull --ff-only origin mlk/master`** — even if already on master, even if status is clean. Pull is non-skippable; upstream may have changes directly relevant to the ticket. **Stash is conditional, NOT default** (refined 2026-05-07 by みや): only stash if the branch has uncommitted work being deliberately preserved (e.g. mid-fix on a feature branch). Default Phase 0 expectation = clean state. If dirty AND on a feature branch with mid-fix work → commit on the feature branch first, then switch + pull. If dirty AND on master with junk → discard or stash explicitly. Stash decisions are deliberate, not automatic — Phase 1 wrap-up is the more typical home for stash. **Sequence when on wrong branch with clean state**: `git checkout mlk/master` → `git pull --ff-only origin mlk/master`. Untracked files (e.g. `.bak_*` files) survive switches — no stash needed. **Why** (2026-05-04 QA #259318): started editing the PRU SKL template while still on `mlk/qa/258418` — would have mixed two tickets' work. **Why** (2026-05-07 QA-260154): ran branch check + status but skipped pull → missed 3 upstream commits including a directly-relevant `TemplateRisalatMMKN_PDT_PLTP.docx` change. Caught only because みや challenged the missing pull. **How to apply**: Phase 0 inventory ritual = (a) Glob etanah-knowledge knowledge files, (b) load Task folder + handoff, (c) **branch-check both repos**, (d) **`git pull --ff-only origin mlk/master` ALWAYS** (after switching to master if needed), (e) only then begin code analysis.
  - **Review the staged diff before committing (hard rule, added 2026-05-18)**: before ANY `git commit` of code, run `git diff --cached` — the FULL diff, not just `--stat` — and actually read every hunk. `--stat` shows file names and line counts; it does NOT show what the lines are. **Why** (2026-05-18 QA-260302): committed `ddfd8ccda2` on a summary of the changes without ever opening the diff; it shipped a dangling XHTML→getter binding plus 2 unused imports. The defect was sitting in the diff — I just never looked. みや had to catch it after the ticket was closed.
  - **Phase 1 close-out — return to master (hard rule, added 2026-05-17)**: after a ticket's `mlk/qa/<num>` branch is committed AND pushed, IMMEDIATELY run `git checkout mlk/master` (+ `git pull --ff-only origin mlk/master`) in that repo. The repo must NEVER be left sitting on a `mlk/qa/*` branch after Phase 1 close — the next ticket's Phase 0 expects clean master. The close-out sequence is: commit → push → `git checkout mlk/master` → pull → THEN run `verify` (Checklist C). The `verify` skill's "repo on master" check is the VERIFICATION; this rule is the ACTION that must precede it. **Why** (2026-05-17 QA-260302): after committing+pushing `mlk/qa/260302`, I left etanah-pelupusan sitting on that branch; みや had to ask why it wasn't switched back — a step that should never need a prompt.
...
- **Layer-aware Phase 0 research — 2-tier ritual + Completion Manifest** (hard rule, 2026-05-07, REDESIGNED 2026-05-08 after みや scrutiny): The etanah codebase is a mature >90% complete system spanning multiple framework layers (Java validators/services/helpers, JSF XHTML/composite/PrimeFaces, Java config/Template-Method overrides, .docx + Word CC, config.json, SQL/Hibernate entities, Spring DI, Flowable BPMN). Fixes are usually small completions of existing scaffolding. Bug shape varies WIDELY by layer; a single rigid checklist either over-fits one ticket type or becomes dead weight on others. Ritual must mirror the architecture: a **universal core** that always applies + **per-layer extensions** that fire only when that layer is involved. **TWO co-equal priorities at Phase 0**: (P1) find the bug + the fix, AND (P2) find an existing tool/utility/validator/set/helper to reuse — both end-to-end verified.
  ...
  **Sub-check 8e — Composite multi-caller verification (added 2026-05-18 after QA-260302 defect #4)**: when a fix adds or relies on an EL binding `#{cc.attrs.<attr>.X}` inside a composite component, `<attr>` (usually `mb`) is NOT one known object — it is whatever each CALLER passes. Verifying the binding on one caller's route ≠ verifying the fix. **Mandatory**: grep `<prefix:compositeName` across `webapp/` for EVERY caller (including indirect callers via a router/dispatcher composite — trace 2 hops), list each caller's `<attr>=` value, resolve each to its backing bean, confirm every resolved bean exposes `X`. Emit one Contract Verification row PER CALLER ROUTE. **Why** (2026-05-18 QA-260302): the Unit-dropdown getter `#{cc.attrs.mb.jenisUnitKadarNilaianSelectItems}` was verified on the direct route (`mb`=`JabatanTeknikalHelper`, getter present) and assumed correct everywhere; on the dispatcher route (`MlkMuatNaikCabutanMinitForm`/`MlkMaklumatCukaiPremiumForm` → `mlkUlasanJPPH.xhtml` → variant) `mb` is the screen bean, which lacks the getter → latent `PropertyNotFoundException`, shipped + committed undetected. **How to apply**: distinct from Check 8 (urusan dispatch) and 8d (BA-scope) — those verify which urusans flow through code; 8e verifies which BEANS back the composite per caller. Any composite reused by ≥2 screens triggers it.
...
*Version: 1.16 | Last updated: 2026-05-19*
