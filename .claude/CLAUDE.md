...
- **🚨 A TEST SCENARIO COMES FROM LIVE TASK STATE, NEVER FROM A ROLE LIST (added 2026-08-05 per みや, #273201).** Before writing any test steps, run ONE query for who holds the tugasan **right now** and hand back that row: `umm_a_tgsn` + `ind_tgsn` + `ind_ursn` + `pcp_pengguna` → `a_tgsn_id · tugasan kod · peranan_semasa · status_tugasan · nama_pengguna`. **Give the LOGIN, never the role.** みや cannot log in as "PPD". **Banned**: deriving a test login from `pcp_capaian_pengguna` / a peranan table / an old BA video — those say who *could* hold it, not who *does*. **Check first whether the app is already parked on the failing step** — usually it is, and the whole setup walk is unnecessary. **Why**: asked for a test scenario, I gave roles (no logins), then logins picked off a capaian role list. Both wrong. The right answer was one query: `a_tgsn_id 2730687`, holder `hadifaizal@melaka.gov.my`, `peranan_semasa -PPTnKanan-PPTT-PPD-`, already sitting on the exact broken branch — zero setup needed. Two corrections, both avoidable.
  - **A failed query is a WRONG QUERY until proven otherwise.** `column "x" does not exist` → get the real column from `information_schema.columns`; `relation does not exist` → schema prefix. **Banned**: reporting a query failure as an environment/DB defect and then proceeding on a guess. Same family as the "DB connection lost" misdiagnosis below. On #273201 I called `umm_a_tgsn` "unreadable on mlit — a DB-side defect" and repeated it three times; the column was `pengguna_semasa_id`, not `pengguna_semasa`, and `SELECT count(*)` had worked all along.
