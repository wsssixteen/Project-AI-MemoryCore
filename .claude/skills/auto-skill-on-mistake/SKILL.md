...
The row is written by `node core/slips.js add …` into `system/slips.jsonl` (`system/slip-log.md` is a FROZEN archive since 2026-07-13 — never append there); the fields that row carries:
...
`system/slip-dashboard.md` (regenerated by `core/slips.js` on every `add`) carries the table:
...
Update mechanism: `core/slips.js` regenerates the dashboard on every `add` (the manual v1 table and the planned `slip-count-tracker.js` hook are superseded; pointer corrected 2026-09-22).
...
**Canonical home**: `system/slips.jsonl` written by `core/slips.js`, surfaced at `system/slip-dashboard.md` (`system/slip-log.md` frozen 2026-07-13; `skill-failure-log.md` deleted 2026-08-16; pointer corrected 2026-09-22).
