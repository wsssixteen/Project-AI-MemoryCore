...
**🚨 ENFORCED (2026-08-20) — `domain/de-knowledge-gate/`**: this etanah-knowledge sweep is no longer optional judgment. A Stop hook fires at the DE-close banner and BLOCKS close when the session produced knowledge-worthy signals (≥3 file:line citations · a research `.md`/`.html` deliverable · a DB MCP query · trace / "how does X work" intent) but no candidate list was emitted. Emit a `## Knowledge candidates` table (`Discovery | Home = bake/defer/drop`) — or the sentinel `_no new knowledge this session_` — before closing. Non-auto-write: みや still approves each bake. Bypass `[skip-knowledge-gate: <reason>]`.
...
> Restored 2026-09-22 (system-check run 3): committed 2026-07-28 (`2940eb07`), silently deleted 2026-08-03 (`25a0379c`) when a stale OneDrive conflict copy overwrote this file before a `git add -A`. The `-PJNBRidhwan` copy preserved it. Boot check `system-audit` CHECK 9 now lists conflict copies so this cannot recur silently.
...
| **Skill-load counter** | Every Skill-tool invocation this session is one row (born 2026-09-07 `domain/skill-invocation-log`, PostToolUse matcher Skill; the "suspended" note stood stale until 2026-09-23 and produced a false audit finding) | `Select-String -Path domain/skill-invocation-log/log.jsonl -Pattern <today>` — rows list skill + args + quest |
...
| **Workflow upgrade for Redmine-Closed quests** (added 2026-09-07, plan §9c — みや: Phase 2 rarely runs straight away, so the learning must not wait for it) | Every quest whose Redmine status is Closed but whose block is still `active`/`closed` gets its 🔧 WORKFLOW UPGRADE search today, even though its Phase-2 archive hygiene may lag | `node quest/redmine-reconcile.js` (lists Redmine-Closed blocks) → for each: `node lib/wrong-fix.js upgrade-table <QA>` → rule every row (`verdict`) and DO the non-none verdicts (knowledge edit / phrase edit / forge) in this DE; emit one line per quest `🔧 <QA>: n rows · knowledge a · phrase b · feature c · none d`. Offline (Redmine unreachable) → emit `⏭ redmine unreachable` and carry to the next DE |
| **Monitoring + housekeeping** (added 2026-09-06, plan §M M6 + system-rules Rule 6) | The turn ledger is consumed and the data lifecycle is applied — the two FIXED housekeeping moments are this step and the system audit | `node lib/turn-report.js` (regenerates `system/monitoring-dashboard.md`: gate yield with true_blocks · cost per quest phase · goal-lens met-rates · overdue watches) → `node lib/goal-lens.js pending` (answer every pending prompt with `note`, or carry to the next DE with a reason) → `node lib/housekeeping.js --dry-run` then `node lib/housekeeping.js` (rotate >8 MB jsonl · delete stale regenerate-class · list consume-class not yet folded; keep-class never touched); paste its summary line into the DE step line |
