...
## Step 12.5 — ⏭ MERGED INTO STEP 12 (2026-05-31)

Meta-audit folded into Step 12 `/verify` Checklist D — both sub-checks that had data sources are now in `/verify`:
- **Cross-reference validity** (INDEX + sub-indexes resolve) → `/verify` Checklist D adds a `meta/INDEX.md` link-resolution row.
- **Component-liveness** (orphaned meta files) → `/verify` Checklist D adds a meta/* orphan-scan row.

Other 3 sub-checks (hook-fire reliability · meta-edit-gate dark · skill-load counter) all depended on files that were DETACHED this session as never-populated stubs (`hook-fire-log.md`, `skill-failure-log.md`); they have no data source. **Re-attach**: only if a deterministic hook-fire writer is built first (parked in todo.md).
...
*Updated 2026-05-31 — **Q2 prune audit applied (haiku Explore audit + verified-redundancy check)**: Step 5 (Forge L1→L2 promotion) ⏭ SUSPENDED — never produced more than 1 L2 in entire history, forge-log.md DETACHED, slip-log running-count covers same purpose. Step 6 (Observation T1→T2) ⏭ SUSPENDED — Observation System DETACHED + all 4 functions (notice/track/integrate/cultural) verified covered by meta/slip-log.md + diary Curated-Index + auto-skill-on-mistake + personality.md Honesty Invariants. Step 7 (Gap Sweep) ⏭ SUSPENDED — almost always 0 gaps surfaced; demoted to on-demand via /appraise or end-of-quest carry-forward; etanah-knowledge sweep preserved as discover-and-route-at-moment. Step 12.5 (meta-audit) ⏭ MERGED into Step 12 /verify Checklist D — 2 surviving sub-checks (cross-reference validity + component-liveness) folded in; 3 dead sub-checks dropped (depended on never-populated stubs hook-fire-log + skill-failure-log). Step 3.5 marker cleaned (removed "pending jsonl + script consolidation" parenthetical; per-quest detail lives in QA-NNN.md). Net boot-token saving: ~4.5k per session. All detaches reversible (no content deleted — markers only).*
