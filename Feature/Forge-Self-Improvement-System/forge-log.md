...
> **🪦 TOMBSTONE (2026-05-24, Phase 8 complete — Task #20):** This file is no longer the canonical home for NEW Forge entries. The new home is **`system/slip-log.md`** under the system-layer's INDEX. Historical entries below remain for reference and have been Bankai-migrated 2026-05-23 (see system/slip-log.md "Historical migrated entries" section). For NEW Forge/slip records → append to `system/slip-log.md` instead.
>
> ---
>
...
| Verify-then-write before applying patterns | rule landed in `.claude/hooks/ticket-gate.js` Phase-0 row #4 ("Working-analog compare") + `.claude/skills/checklist/SKILL.md` "Item source — independent enumeration" | **2** | ↑ from L1 on 2026-05-20 per みや. **Original** 2026-04-30 — applied option (c) `<et:formField label="">` for QA #258418 placement WITHOUT grepping codebase first to see if `label=""` pattern exists. Zero matches. JBoss runtime: `ComponentNotFoundException`. Reverted. Pattern: assumption-then-write. **Recurred 2026-05-20** (QA-262039 `kadarCukaiPT` slip — recommended a tag by pattern-matching the rendered output string, without checking the working sibling-template that would have shown PSBS uses `hasilTahunPertamaWithRM`). Structural fix landed: working-analog-compare is now Phase-0 row #4 in `ticket-gate.js` reminder + content rule in `checklist` skill. Watch L2→L3 in next 3 sessions: does the next ticket invoke working-analog FIRST without prompting? |
| **Enumeration completeness — "every X, not the X"** | `.claude/skills/checklist/SKILL.md` "Enumeration completeness" section + `.claude/hooks/ticket-gate.js` Phase-0 row #2 | **1** | New 2026-05-20 (name per みや's call). Pressure-test across QA-262027 (BA-numbered items #5/#6 lived in prose footnote, not as rows) + QA-260965 (`initWarna()` page-load writer missed because Phase 0 traced "the" handler, not every writer) + QA-260154 (double-gate trap: outer flag + inner gate; Check 4 asked for "the gate" singular) all failed in the SAME shape. Phase 0's Universal Checks ask for "the X" too often where "every X" is what closes the gap. The full axes: ALL BA-numbered items / ALL gate-variable writers (init / populate / event-handler / page-load) / ALL OR-bypass conditions in validator chains / ALL data-axis branches (input values that flip the conditional) / verbatim-body-read of every cited method / Requirement parents fetched via the cross-ref agent. Saved into the `checklist` skill. **Watch L1→L2** in next session: does the next ticket's Phase 0 emit the fan-out checks without reminder, or enumerate partial again? |
...
| Mandatory quest-step silent-skip | `quest/quest-protocol.md` (Phase 0 artifact gate + verify-close re-commit clause, v3.1) + `.claude/hooks/ticket-gate.js` (visible 6-row Phase-0 checklist reminder, deterministic UserPromptSubmit injection — 2026-05-20) | **1** | 2026-05-18 — QA-260302: `early-diagnostic.md` never created despite the Auto-Discovery rule, AND state files never reconciled after the commit was re-done — both silent skips, unnoticed 5 days. Fixed with the visible-gate pattern (Phase 0 emits `early-diagnostic.md ✓`; verify-close re-runs on any re-commit). **2026-05-20** — same shape recurred (QA-262039: I refined the `checklist` skill mid-session, then never created an Issue Checklist for QA-262039 itself — the exact slip the skill exists to prevent). Structural fix: `ticket-gate.js` Phase-0 reminder now injects the 6-row checklist deterministically on quest signals (hook-level, not skill-judgment). **Stays L1 per みや 2026-05-20** — watch L1→L2 across 3 more sessions before promoting. |
...
| "Always produce class chains when tracing code" | 2026-04-15 | High-value rule, but sometimes a one-line answer is sufficient and the chain becomes ceremony. Need a "when chain is overkill" clause. | **Refined 2026-05-22** — reworded "always" → triggered form ("when execution flows across ≥2 classes/files"); migrated to `personality.md` Communication: DO during the CLAUDE.md decomposition. The "when-overkill" concern is resolved — the trigger now scopes it (single-file / non-code answers don't fire). |
