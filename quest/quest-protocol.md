...
1. **Spawn a familiar** (Agent with `general-purpose` subagent) — its prompt must include:
   ...
   - **Severity-downgrade is itself a claim (added 2026-06-14, QA-260508 cycle-4 — the B2 refine)**: the inverse move — *downgrading* a previously-flagged suspect to "dead code / not critical / not the cause" — is ALSO a claim, not an observation, and needs the same discharge as an `(inferred)` tag: a traced path (init/call-path via `codegraph_trace`, or a quoted `file:line`) cited in the SAME emit. A glance-based downgrade is BANNED. If a みや minimal-repro contradicts the downgrade, the **repro outranks the static read**. **Why**: QA-260508 — flagged the `:173` `denda` local-var shadow as a real bug, then downgraded it to "dead code, not critical" with no init-path trace, then みや's "page-load → straight Simpan, no popup" repro proved it WAS the cause. Two confident reversals in one session = anchoring; treating the downgrade as a claim-to-discharge kills it.
...
**🔬 Cycle≥2 = probe-first before any fix Edit (the B5 refine, added 2026-06-14, QA-260508 cycles 1-2).** On any rework cycle (cycle≥2 — `active.txt` shows the quest was `closed`/`archived` and reopened, OR a `3. Rework/` / `4. Rework/` subfolder exists), a fix Edit is BANNED until a **runtime probe produced ground truth THIS cycle** — UNLESS the fix is **template-only / config-only / syntax-only AND code-reviewable without execution** (the exception class). Two prior static-reading fixes already failed; cycle-2+ does not get a third static guess. **Why**: QA-260508 cycles 1-2 both shipped static-read fixes (wrong writer, then stale-VO wipe) that failed test; cycle-3's probe-first found the real two-helper split in one pass. **Exception examples** (probe adds no value — static read IS conclusive): QA-262762 FONT-uppercase · QA-262027 CC-retag · QA-262004 doc-list populator. **Enforcement**: prose + exception class NOW; a `quest-phase-gate.js` rework-probe extension is DEFERRED pending a 2-3-ticket trial confirming the exception class doesn't over-fire (~43% of past cycle≥2 rework was template-only). **Banned**: a fix Edit on a behaviour-bug rework cycle with no runtime probe THIS cycle and no exception-class justification.
...
- **Deploy-proof at every source-edit hand-back (added 2026-06-14, QA-260508 S3 — the B4 refine).** NEVER hand back "please test" / "ready for verification" / "try testing" after a `.java`/`.xhtml`/`.docx` edit without stating HOW a pass/fail will be unambiguous — one of: **(a)** probe assertion — "grep `QA<num>-PROBE` in server.log; presence = new bytecode is live"; **(b)** a build/republish timestamp to confirm before testing; **(c)** an explicit "Eclipse → Clean → Republish first" instruction. **Why**: QA-260508 — said "please test" after a source edit without verifying the running JVM held the new bytecode; the failed test was ambiguous (fix-wrong vs stale-deploy) until the probe disambiguated. The probe doubling as deploy-proof IS the pattern. **Banned**: "please test" after a source edit with no deploy-proof artifact. (Enforced deterministically by `deploy-proof-gate.js` Stop hook.)
...
*Protocol version: 3.9 — 2026-06-14 (QA-260508 B-refines: **B2** severity-downgrade-is-a-claim added to the Recon observed-vs-inferred honesty primitive; **B4** deploy-proof-at-source-edit-hand-back bullet added to Ritual 6 [paired with new `deploy-proof-gate.js` Stop hook]; **B5** cycle≥2 probe-first + template/config/syntax-only exception class added to the Rework-restart rule [hook deferred]. Companion CLAUDE.md §10 edits = B1 Predicate-Diagram probe-discharge + B3 sibling-diff VO-instance-PROBE-REQUIRED. Full rationale: meta/claude-md-changelog.md 2026-06-14 + system-architecture.md v1.8 sync event.)*
