...
>
> **🌐 META-LAYER NOTE (added 2026-05-23, Phase 9 integration):** Quest is a Layer 2 workflow that COMPOSES Layer 3 capabilities (skills + hooks). Several disciplines historically embedded in this protocol have been **hoisted to atomic primitive skills** under the system-layer (Layer 1). Quest invokes them by name; the underlying behavior lives in the primitive's SKILL.md.
>
...
>
> See `system/INDEX.md` + `system/discipline-INDEX.md` + `system/honesty-INDEX.md` + `MIYA-NOTEBOOK.md`.
...
**Compound trigger — "wrap + commit prep + close" (added 2026-05-12, pull-step corrected same day)**: Recognize ANY combination of these phrases as a Phase 1 full close-out request — auto-fire the entire flow (stash → **pull --ff-only origin <source-branch>** → branch → pop → add → propose commit message → みや confirms the message → Ruri runs commit + push → **[merge → mlk/int-env + push int-env, when the ticket deploys to internal]** → **🚨 return-to-`mlk/master` (LAST, non-optional — the int-env merge leaves you ON int-env; this is the step that kept getting dropped, per みや 2026-08-10)** → pull → update active.txt → `/verify`). Verify `git branch --show-current` = `mlk/master` before declaring done; **banned**: ending on `mlk/int-env` or the ticket branch. **🚨 The pull between stash and branch is mandatory** — see line 75 hard rule. Never paraphrase this sequence without the pull; both today's tickets (QA-259318 v2 and QA-260179) had it dropped in the announcement (master happened to be at-tip so no merge conflict, but it's a stale-base risk we don't take):
