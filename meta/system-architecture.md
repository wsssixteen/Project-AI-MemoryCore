...
| `auto-commit-docs.js` (+`auto-commit-worker.js`) | self (NEW 2026-07-01, per みや /goal — auto-save MemoryCore docs) | every Stop — dispatches `auto-commit-worker.js` in the background to commit+push tracked MemoryCore docs | background commit+push of tracked MemoryCore docs; **MemoryCore-only, etanah HARD-guarded** (never touches etanah repos); templated commit message v1 | `feedback_daily_commit.md` (MemoryCore commit=default), DE commit/push step; worker is invoked by the hook, not separately registered | etanah-guard must stay load-bearing; message template tunable |
