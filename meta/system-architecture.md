...
|---|---|---|---|---|---|
| `ticket-gate.js` | self | ticket number patterns — QA/FAT-OR/UAT-CR/FAT/UAT **+ REQUIREMENT/REQ/CR/Redmine/ticket/issue #N (broadened 2026-06-23)** + Redmine-retrieval (verb/noun non-adjacent, 2026-06-23) | injects Phase-0 gate checklist; **Row 0 (2026-06-20) = COMPULSORY GIT-STATE CHECK** — baseline-verify + behind-origin count + existing-fix probe; **Row 0.5 (2026-06-23) = CLASSIFY MODE debug\|development** (auto-infer + みや confirms; dev → forward Goal+Plan via `writing-plans` PLAN.md / QA-NNNN.md section). Decouples the requirement-checklist from `/quest start` → fires on ANY ticket engagement | Quest skill, Phase-0 git-discipline, quest-MODE system (components 3 dev-plan-wiring + 4 hard-gate PENDING — todo.md Q1) | regex update if ticket format expands; Row-0 git-commands per repo baseline |
