...
>
> Version: 1.8 — created 2026-05-27 from Plan `cached-floating-hummingbird.md` Phase 0. Sync events list at footer (latest: 2026-06-14 QA-260508 B1-B5 + B6 dead-hook finding).
...
| `silent-claim-drift-gate.js` | self | "done"/"complete" claims without diff-backing · **Ext D severity-downgrade-without-trace (B2, 2026-06-14)** | advisory reminder (Stage 5A) / hard-block (Stage 5B) | Phase 5 enforcement, claim-verification skill, B2 refine | Phase 3 + Phase 5 + Ext D extend this hook. ⚠️ Was syntax-DEAD in HEAD pre-2026-06-14 (`.skills/*/SKILL.md` `*/` closed the header comment → parse error → fail-open) — fixed 2026-06-14 |
| `deploy-proof-gate.js` | self (NEW 2026-06-14, B4 refine) | test hand-back ("please test") after an etanah source Edit with no deploy-proof artifact | advisory reminder (Stage 5A) | quest-protocol Ritual 6, QA-260508 S3 | Stop hook; block-stage flip deferred to post-observation |
...
| 2026-06-14 | v1.8 — **QA-260508 B1-B5 weekend refactor** (plan `radiant-chasing-abelson.md`, routed through system-design): B2 severity-downgrade-is-a-claim (quest-protocol Recon primitive + `silent-claim-drift-gate.js` Ext D) · B4 deploy-proof (quest-protocol Ritual 6 bullet + NEW `deploy-proof-gate.js` Stop hook, registered in settings.json) · B1 probe-before-VO-fix + B3 two-VO disambiguation (CLAUDE.md §10 Predicate Diagram falsifier-branch + sibling-diff EMIT LINE) · B5 cycle≥2 probe-first + exception class (quest-protocol Rework-restart; quest-phase-gate hook DEFERRED pending 2-3-ticket trial). **B6 FINDING**: `silent-claim-drift-gate.js` was syntax-DEAD in HEAD (header-comment `*/` parse error → fail-open → zero enforcement for weeks) — fixed; a `node --check` hook-syntax sweep proposed (todo Q1) since `meta-layer-audit.js` checks registration not runtime validity. wrong-baseline-diagnosis 🚨🚨 + tool-choice-skip 🚨 defenders SHIPPED. | QA-260508 weekend refactor |
