...
| `meta-edit-gate.js` | self | `meta/*` path edits + (Phase 0 extension) hooks/skills/protocols/state-files. **v1.3 (2026-07-20): `.claude/hooks/*.js` added to systemTouchingPatterns** — the largest hook dir was uncovered, so edits there took the advisory branch and LANDED before design, forcing a write-then-revert. Now hard-denied like `domain/*` hooks. | recursive safety + paired-edit predicate | meta-layer audit, architecture-doc-sync | Phase 0 extends this hook for architecture-doc-sync |
...
- Net Stop-hook delta: 20 → 21 (this Feature added — replaces the previously-un-catalogued advisory hook that lived at `.claude/hooks/`). §3.0 auto-registry regeneration deferred to a merge-to-main follow-up (`node meta/sync-hook-catalog.js`).

### 3.16 Change 2026-07-06 (design-consult-gate v1.2 + meta-edit-gate arch-doc-sync PROMOTED — background task)
...
- 🔁 **EXTENDED `domain/design-consult-gate/design-consult-gate.gate.hook.js` → v1.2** (routed /system-design + /system-rules; merge-in-place per Rule 2 — did NOT create a new hook). Root slip: 2026-07-06 QA-268883 session shipped 5 framework files (enum entry in `WordContentControlTypeEnum.java`, 2 util methods in `PelupusanUtil.java`, dispatcher case + handler in `PelupusanTemplateUtil.java`, helper in `PelupusanWordEditorUtil.java`, populator flip in `PelupusanWordCCMethodConstant.java`) with ZERO `/system-design` invocation because the gate's guarded paths did not cover etanah code AND did not cover the top-level rule files themselves.
  - ➕ Guarded paths extended from 3 → 9: added `.claude/settings.json` · `CLAUDE.md` · `personality.md` · `meta/**` · `quest/quest-protocol.md` · `domain/*/*.skill.md` (was `.hook.js` only).
  ...
  - 📐 **Smoke-tested 8/8** (fire check + effect check per Rule 6 v1.2): guarded CLAUDE.md/meta/quest-protocol/settings.json/personality.md all deny; unrelated file passes; etanah NEW method advisory; etanah line-fix silent.
- 🔁 **PROMOTED `.claude/hooks/meta-edit-gate.js` v1.1 → v1.2**: architecture-doc-sync predicate advisory → **HARD-BLOCK** (`decision:deny`). System-touching edit (hook / skill / Feature hook / quest-protocol / active.txt / settings.json) is denied unless `meta/system-architecture.md` was Read or Edit'd earlier this session (transcript match) OR bypass `[skip-architecture-doc-update: <reason>]` present. Advisory reminder for other meta-layer paths preserved unchanged. Smoke-tested 5/5 (block without arch-doc-read · pass with arch-doc-read · pass with bypass · pass when editing arch-doc itself · no-op on non-system-touching).
