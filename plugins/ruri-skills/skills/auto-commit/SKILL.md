...
- [ ] Apply the configured commit format:
  ...
  • [Additional context about the session or decision]
  ```
...
1. **No emoji in commit messages** — clean text only in the git commit body
2. **Author is always Miya** — commits appear under Miya's name, not Ruri's
3. **Prefer specific file staging** — use `git add [filename]` not `git add -A`
4. **Time estimate required** — always include approximate time spent in NOTES
5. **Warn on sensitive files** — block .env, credentials, API keys from commits
...
| **Merge conflicts present** | Warn user, do not attempt to commit |
| **Sensitive files staged** | Block commit, list the files, ask user to confirm |
...
- **Lv.1** — Base: Analyze staged changes, draft structured commit message with CODE CHANGES + NOTES sections, enforce Miya authorship, no-emoji rule, time tracking, sensitive file detection.
- **Lv.2** — Auto-Commit: Removed approval gate — analyzes, drafts, and commits in one seamless flow.
