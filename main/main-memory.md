...
**Session 2026-04-28 (weekday, afternoon)**: QA #258022 — Utiliti Semakan Borang missing Pembetulan + Agihan Kepada fields. Two root causes identified and fixed. Root cause 1: no config entry for tugasan SB4CE + Utiliti urusan in `tindakan.config.json` → added `tugasanSB4CE_UTILITI` entry with `option_type: smb_all`. Root cause 2: `MlkMaklumatUrusanPermitForm.getAdaPegawaiAgih()` compared against `URS_PRBB` ("PRBB") while Utiliti uses "OPRBB" etc. — fixed getter (URUSAN_LITE_LIST.contains) + initialized `adaPegawaiAgih = true` in `initRenderPanel()` for SB4CE. Supporting: added `TGSN_PENGESAHAN_BORANG_ALL`, `TGSN_PENYEDIAAN_BORANG_ALL`, `TGSN_SEMAKAN_BORANG_ALL` ImmutableSet constants to `PelupusanTugasanConstant`; simplified Codex's inline OR chains in `BasePelupusanLiteForm` to use them. Codex's wrong-bean (`MlkPenyediaanBorang4CeP1eForm`) reverted — it was Penyediaan not Semakan. Codex's service changes (`MlkPelupusanPegawaiAgihService` SB4CE routing) kept. Deep scrutiny discussion: みや asked to evaluate every Codex change before accepting — appraise is better than simplify when external code correctness is unverified. Pending: FAT test on OPRBB Semakan Borang step.
