...
**⚠️ Parallel-session note (2026-05-28)**: TWO independent sessions ran today and both wrote this RAM file — (a) the **diary-redesign** session (worktree `modest-saha-8e8678`: Phase 1+2+3+side-quest, then session-keyword-tracker REMOVED at リドワン's request after the tracked-JSONL dirty-tree flaw surfaced) and (b) the **Quest-workflow-refactor** session (recorded below). Both records preserved; they merged at DE 0b. This is itself evidence of the RAM-file-can't-hold-two-parallel-sessions limitation — a structural item for the future "how best to structure this" redesign. **Latest activity: diary-redesign session's 3rd DE close (tracker removal), ~11:10 MPST.**
...
- 📌 **DE architecture: hook+docs vs first-class Skill** (NEW 2026-05-28) — Domain Expansion is currently a HOOK (`domain-expansion-trigger.js`) + protocol doc + feedback memory, NOT a Skill tool skill. Banner-confabulation was fixed by inlining the canonical banner into the hook (commit `554bc01`) — that closed the determinism gap (the lever is inline-at-trigger, NOT hook-vs-skill; a hook with inlined content IS deterministic). OPEN QUESTION for future eval: should DE become a first-class Skill? NOT for banner-determinism (inline solved that) but because the 12-step ritual is complex procedural logic (conditional skips, sub-agent orchestration, /verify Checklist D) that a Skill might orchestrate more robustly than a text-injecting hook. Trade-off: a Skill adds an invocation requirement (the weak link the skill-invocation-discipline hooks exist to police). Evaluate via /system-design + /grill-me next session. Cross-ref: meta/slip-log.md 2026-05-28 DE-close entry; meta/system-architecture.md v1.3.
- 📌 **Phase 5 Stage 5B flip** (NEW 2026-05-28) — `silent-claim-drift-gate.js` + `PlainFirstGate.js` are in Stage 5A advisory mode. Flip to `decision:"block"` hard-block ONLY after 1-2 sessions observing false-positive rate. Dry-run scenarios at `meta/hook-test-scenarios.md`. Do NOT flip blind.
- 📌 **rubric stub decay 2026-06-07** (NEW 2026-05-28) — standalone rubric skill is a redirect stub; `worktree-cleanup-boot.js` v1.1 will flag it from 2026-06-04 onward (3 days before). Final cleanup (delete dir + absorb triggers) due 2026-06-07.
- 📌 **First boot validation of new invariant scanner** (NEW 2026-05-28) — `meta-layer-audit.js` v1.1 INV-3..INV-6 checks have NEVER run against a real boot. Next SessionStart is the first live test — watch the `🛡 invariants: N/6` line. If it crashes or false-positives, that's the first thing to fix.
