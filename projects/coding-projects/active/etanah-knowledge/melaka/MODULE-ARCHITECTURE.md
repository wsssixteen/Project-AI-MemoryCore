...
### Document vs Report — two independent subsystems; "Papar" preview is a live REPORT, not the stored document (added 2026-06-08, QA-262762)

**Plain explanation**: A tugasan's on-screen "Papar &lt;borang&gt;" preview and the stored document that later tugasans print are TWO different subsystems. The preview regenerates live from current data on every click; the stored document is a cached, versioned record. So editing data + reopening the preview is always fresh **without** touching the stored record — the preview never reads it.

| Subsystem | Artifact | Generated by | Refreshes how | Consumed by |
|---|---|---|---|---|
| **Report** (preview) | transient PDF (`BaseReportPropertyJson`) | `initReport()` → `PelupusanReportUtil.processReportListConcurrently(…, false, …)` | **live, every "Papar" click** (`false` = preview, not persisted) | the "Papar" dialog on the data-entry form |
| **Document** (stored) | `AppDokumenKeluaran` (+ DMS `Document`, versioned) | `BasePelupusanDokumenForm` framework (`processTemplateList` → `PelupusanTemplateUtil.processTemplate…`) | regen-if-missing on doc-screen open (`refreshDokumenList` / `updateDocumentListAndProcessTemplateIfNotAvailable`); versioned regen via `regenerateNewDocument` (jana semula); invalidate via Form-layer post-submit hook | downstream Cetakan / Pengesahan tugasans |

**Populators**: REPORT path = `PelupusanReportMethodConstant`. DOCUMENT (Word-template) path = `PelupusanWordCCMethodConstant`. Don't confuse them when a "borang display" ticket lands.

**Layer rule**: document/report lifecycle (generate / regenerate / invalidate) belongs in the **Form layer** (`Base*Form` / `overridePostSubmitMethod`), NOT a **data-save service** (`PelupusanService.save…`). A save persists data and stops.

**Implications for fixes**:
- "Papar &lt;borang&gt; shows stale after edit" → fix the REPORT populator (`PelupusanReportMethodConstant`) / live report path — NOT the stored document. Never delete `AppDokumenKeluaran` to "force a refresh"; the preview doesn't read it.
- "Document missing on the next tugasan" → something deleted or never-generated the stored `AppDokumenKeluaran`. Look for a stray delete — especially one wrongly placed in a save service.
- To refresh a STORED document, use versioned `regenerateNewDocument` (jana semula) or the Form-layer post-submit invalidation — never a record delete in a data-save method.

**Why this matters / how it bit** (QA-262762 + #264312): a delete of `AppDokumenKeluaran` (BORANG_4AE) was placed in `PelupusanService.saveTujuanPermohonanPermitLesen` to "make the Borang reflect changes" — but "Papar Borang 4Ae" is a live report that never reads that record (so the delete did nothing for the preview), while the Cetakan tugasan DOES read it (so the delete made the doc go missing → #264312). Correct fix = pure removal of the delete; the real display fix was always the populator.

**Process lesson**: before theorizing a document fix, OBSERVE/trace the actual button → method (one xhtml grep on the `action=` binding) to learn which subsystem it uses. Two cycles of wrong fixes here came from *assuming* the data source instead of tracing it.

**File:line references**:
- Report preview: `MlkPenyediaanBorang4AeL1eForm.xhtml:290` (`papar-borang-4ae` → `mb.initReport`) → `BasePelupusanForm.initReport():563` → `PelupusanReportUtil.processReportListConcurrently(…, false, 4)`
- Document framework: `BasePelupusanDokumenForm` — `regenerateNewDocument():412`, `refreshDokumenList():619`, `updateDocumentListAndProcessTemplateIfNotAvailable():565`, `processTemplateList():512`
- Invalidation convention: `MlkLaporanL1eForm.overridePostSubmitMethod():202` (delete-at-Pengesahan on `pembetulan`)
- The anti-pattern: cycle-1 `f4a73be3cc` (delete in save service); reverted `7ba00c2` (#264312)
