...
| [BRANCH-AND-DEPLOY.md](BRANCH-AND-DEPLOY.md) | Branch topology (nothing ever merges into `mlk/master`; ticket branch forks to `int-env`/`stag-env`/`release/<ver>`) + the 2 deploy servers and the internal-vs-staging pipelines | You need to get a fix onto MLIT or MLKSTAG, or you need to know whether a branch has actually shipped. Use `/deploy` rather than re-deriving |
