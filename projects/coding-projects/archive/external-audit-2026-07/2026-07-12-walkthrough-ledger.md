...
| **Deferred — added 2026-07-13 morning after みや's gap question (were MISSING from this table — my omission)** | **C4 skills 48→~30 refactor** (not started; dedicated session) · **close.js + close skill** (DE recomposition — kernel piece, one-per-session rule) · **K2 full migration artifacts** (validator exists, jsonl+schemas+single-writer gate don't) · **K5 lifecycle.js module** (flags live in telemetry-report only) · per-🚨 replay fixtures beyond block-capable set · N3 stream-rules · N5 [V]/[H] tags · N6 live boot-budget gate · N1 mechanization · C6 doc-table generation |
