...
*Melaka profile · v3.8 (2026-08-27): Map tab slimmed to a read-only swimlane OVERVIEW — Bands mode + Pin/Drag/Reset + help-bar + legend + side panel REMOVED (stale localStorage pins skipped layout and scattered new tables — the "broken Bands" report); clicking a Map card now jumps to the Tables focus (one inspection surface); Map edges now include dashed implicit links (DATABASE.md §10b truth). Prior: v3.7 (2026-08-23): Tables tab = Diagram/Catalog/By Urusan/By Feature sub-tabs · code-truth scope (pelupusan=151 tables, 7-method scan + adversarial workflow) · 596-entity registry · 1,433-tugasan census · 46 screen traces · 85 families · sidebar tabs · atlas-ship-gate Stop hook enforces smoke+real-render before any session stops. mapping v3.0 (2026-08-22) · 781 tables · 2,146 FKs · 350 implicit links · 12 moduls · 13 urusans (all with BPMN-verified decision forks)*
*v4.1 (2026-08-27): MULTI-STATE — 5 self-contained state HTMLs (Melaka·Selangor·Perak·Terengganu·WP) with a header STATE switcher, each built LIVE from its own DB (Postgres + Oracle). Binding rules + per-state verification ledger now live in `SPEC.md` §5; build/add/verify steps in `README.md`. Journey/By-Feature/code-truth remain Melaka-only (per-state urusan curation pending). For behaviour rules read SPEC.md FIRST — it supersedes any stale description below.*
