...
function renderUrusanView() {
  ...
  const cols = list.map(kod => {
    ...
    // Group siblings that are the SAME task by different type/role (e.g. the 8
    // "Penyediaan Laporan Tanah <CODE>" rows) into ONE line; the codes/roles show as a
    // small inline hint — no dropdown. A lone sub-flow keeps its own steps-dropdown.
    const baseNameOf = (name) => {
      let s = String(name || "").replace(/^\d+(\.\d+)?\s*/, "");   // leading "5.0 "
      s = s.replace(/\s*\([^)]*\)\s*$/, "");                        // trailing "(MLK_...)"
      s = s.replace(/\s*[-–]\s*[A-Z]{2,4}(\s*\([A-Z0-9]{2,6}\))?\s*$/, ""); // "- PDT" / "- PTG (KPT)"
      s = s.replace(/\s+[A-Z0-9]{3,}$/, "");                        // trailing ALL-CAPS code "PJTPLT"
      s = s.replace(/\s+\d{1,2}$/, "");                             // trailing instance number "Jabatan Teknikal 4"
      return s.trim();
    };
    const variantOf = (name) => {
      const noMod = String(name || "").replace(/\s*\([^)]*\)\s*$/, "");
      let m = noMod.match(/\s+([A-Z0-9]{3,})$/);
      if (m) return m[1];
      m = noMod.match(/[-–]\s*([A-Z]{2,4})\s*$/);
      return m ? m[1] : null;
    };
    const groupRows = (rows) => {
      const order = [], map = new Map();
      for (const r of rows || []) {
        const b = baseNameOf(r.name) || r.name;
        if (!map.has(b)) { map.set(b, { base: b, rows: [], variants: [] }); order.push(b); }
        const g = map.get(b);
        g.rows.push(r);
        const v = variantOf(r.name); if (v && !g.variants.includes(v)) g.variants.push(v);
      }
      return order.map(b => map.get(b));
    };
    const ujGroup = (g, color) => {
      if (g.rows.length === 1) return ujRow(g.rows[0], color);   // lone task/sub-flow → normal (keeps dropdown)
      const r0 = g.rows[0];
      const st = color ? ` style="--fork-color:${color}"` : "";
      const allCall = g.rows.every(r => r.kind === "callActivity");
      const badge = allCall ? `<span class="uj-seq-mod">${escapeHtml(r0.module || "")}</span>` : "";
      const pers = [...new Set(g.rows.map(r => r.peranan).filter(Boolean))];
      const per = pers.length ? `<span class="uj-seq-per">${escapeHtml(pers.join("/"))}</span>` : "";
      const shown = g.variants.slice(0, 6);
      const more = g.variants.length - shown.length;
      const hint = g.variants.length
        ? `<span class="uj-seq-variants">${shown.map(escapeHtml).join(" · ")}${more > 0 ? ` +${more}` : ""}</span>`
        : `<span class="uj-seq-per">×${g.rows.length}</span>`;
      const head = `<span class="uj-seq-name">${escapeHtml(g.base)}</span>${badge}${per} ${hint}`;
      // merged sub-flow (same sub-process called several times) → ONE line + ONE dropdown of its inner steps
      const subUnion = [...new Set(g.rows.flatMap(r => r.sub_tasks || []))];
      if (subUnion.length) {
        return `<details class="uj-task uj-task-sub uj-task-group"${st}><summary>${head} <span class="uj-seq-per">${subUnion.length} steps</span></summary>${subUnion.map(x => `<div class="uj-seq-sub">↳ ${escapeHtml(x)}</div>`).join("")}</details>`;
      }
      return `<div class="uj-task uj-task-group"${st}>${head}</div>`;
    };
    ...
    u.stages.forEach((s, si) => {
      ...
      const groups = groupRows(dedupeRows(rows));
      const stepsBlock = groups.length
        ? `<div class="uj-tasks">${groups.map(g => ujGroup(g, null)).join("")}</div>`
        : "";
      parts.push(`
      <div class="uj-stage">
      ...
      if (s.fork && (s.fork.outcomes || []).length > 1) {
        ...
        if (branchRows.length) {
          ...
          const branchG = groupRows(dedupeRows(branchRows));
          parts.push(`<div class="uj-tasks uj-fork-tasks" style="--fork-color:${oc}">${branchG.map(g => ujGroup(g, oc)).join("")}</div>`);
          if ((seq.converged || []).length) {
            const convG = groupRows(dedupeRows(seq.converged));
            parts.push(`<div class="uj-conv-label">— shared tail (runs on every outcome) —</div>`);
            parts.push(`<div class="uj-tasks">${convG.map(g => ujGroup(g, null)).join("")}</div>`);
          }
        } else {
