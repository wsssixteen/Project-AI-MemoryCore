...
runHook({ name: 'local-deploy-gate', event: 'UserPromptSubmit' }, (input) => {
  ...
  return {
    ...
    contextOut: [
      ...
      '',
      '   KNOWN ROOT CAUSE (2 occurrences): WEB-INF/jboss-deployment-structure.xml missing from',
      '   the DEPLOYED war. It declares <module name="org.hibernate"/> and lives ONLY in the',
      '   etanah-common WAR overlay. Overlay fails to merge -> file absent -> Hibernate/Spring',
      '   ClassNotFoundException. Overlay resolves via the M2_REPO classpath variable, which m2e',
      '   derives from the active settings.xml <localRepository>.',
      '     E:\\Dev\\.m2_etanah = real repo (8.47 GB, holds the overlay wars)',
      '     E:\\Dev\\.m2        = decoy (near-empty, only .lastUpdated markers)',
      '',
