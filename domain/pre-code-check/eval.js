...
// same pass.
const GOOD_PREFIX = 'CODE-CHECK: analog ✓(convertPdfToImages():1496-1513 in-file) · in-file ✓ · sibling ✓ · existing-reuse ✓(reuses writeJpeg():1550) · name-by-purpose ✓ · minimal-diff ✓ · logic-matrix ✓ · blast-radius ✓(grep callers PelupusanUtil.java:886/:888) · predicate ✓ · falsifier ✓(regenerated doc size band check) · read+write-path ✓(no persist, bytes embed only) · full-address ✓ · sibling-diff ✓ · all-writers ✓(grep setAlamatBerdaftar -> 4 sites each init-safe) · kod-resolution ✗(N/A — no kod literal) · prior-fix ✓(git log -S 150 DPI -> 0 hits) · class-chain ✗(N/A — no inherited member) · peranan-map ✗(N/A — image rendering, no role) · flowable-contract ✗(N/A — no tugasan submit path) · fallback-precedence ✗(N/A — no fallback/default branch in this diff)';
