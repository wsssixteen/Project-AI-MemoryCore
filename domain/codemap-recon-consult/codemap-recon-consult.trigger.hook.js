...
process.stdin.on('end', () => {
  try {
    ...
    const reminder = [
      ...
      '  • entity_table_map.json — 596 entity↔table (etanah-domain-1.0.4-MLK JAR). Resolve a',
      '          table\'s entity (or the reverse) HERE first — never grep the table name in Java',
      '          (@Table names are prefix-SPLIT: TablePrefixConstant.UMM_TABLE_PREFIX + "A_HKMLK").',
      '  • features.json      — 12 verified feature groups + grep-verified class chains (templates ·',
      '          reports · flowable · tugasan · screens · emohon · spoc · dokumen · persistence ·',
      '          peranan · migration · config). Working-analog picking + feature-level blast radius:',
      '          the sibling to copy is usually inside the same feature group. (built 2026-08-23)',
      '',
