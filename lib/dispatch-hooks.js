...
// M1/M2 turn context (plan §M, 2026-09-06) — same fail-open shape as lib/hook-runtime.js.
let turnCtx = null;
try { turnCtx = require(path.join(__dirname, 'turn-context.js')); } catch (_) { turnCtx = null; }
try { if (turnCtx) turnCtx.openTurn(input, EVENT); } catch (_) {}
const CTX = (() => { try { return turnCtx ? turnCtx.contextFor(input) : {}; } catch (_) { return {}; } })();
function firstLine(s, n) { const l = String(s || '').split('\n').map(x => x.trim()).filter(Boolean)[0]; return l ? l.slice(0, n || 160) : ''; }

// Orchestration mode (2026-08-16, miya-approved retry): while a sweep runs, the six
// reply-FORMAT checkers below stand down (they misjudge coordinator messages as code
// work). Honesty/claim checkers are NOT in this list and never stand down. The flag
// file system/orchestration-mode.flag carries an expiry timestamp (max a few hours,
// written by the sweep skill) so a crashed sweep can never mute checks permanently.
// Every skipped check writes a telemetry row (mode:'orch-suppressed') — visible, never silent.
...
function orchestrationActive() {
  try {
    const exp = parseInt(fs.readFileSync(path.join(ROOT, 'system', 'orchestration-mode.flag'), 'utf8').split('\n')[0], 10);
...
function runChild(rel) {
  return new Promise(resolve => {
    ...
    const t0 = Date.now();
    const stem = path.basename(rel, '.js').replace(/\.(gate|check|trigger|discipline)?\.?hook$/, '');
    if (ORCH_SUPPRESS.has(stem) && orchestrationActive()) {
      appendTelemetry({ ts: new Date().toISOString(), hook: path.basename(rel, '.js'), event: EVENT, mode: 'orch-suppressed', exit: 0, blocked: false, dur_ms: 0 });
    ...
    const p = spawn(process.execPath, [abs], { windowsHide: true, env: { ...process.env, CLAUDE_PROJECT_DIR: ROOT } });
    let stdout = '', stderr = '';
    ...
    p.on('close', code => {
      ...
      const fired = blocked || stdout.trim().length > 0;
      let reason = '';
      if (blocked) {
        const m = stdout.match(/"(?:reason|permissionDecisionReason)"\s*:\s*"((?:[^"\\]|\\.)*)"/);
        reason = firstLine(m ? m[1].replace(/\\n/g, '\n') : (stderr || stdout), 160);
      } else if (fired) {
        const m = stdout.match(/"additionalContext"\s*:\s*"((?:[^"\\]|\\.)*)"/);
        reason = firstLine(m ? m[1].replace(/\\n/g, '\n') : stdout, 160);
      }
      appendTelemetry({ ts: new Date().toISOString(), hook: path.basename(rel, '.js'), event: EVENT, mode: 'bundle', exit: code == null ? 0 : code, blocked, fired, dur_ms: Date.now() - t0, ...CTX, ...(reason ? { reason } : {}) });
