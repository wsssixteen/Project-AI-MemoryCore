...
function main() {
  ...
  for (const name of listDir(path.join(ROOT, 'domain'))) {
    ...
    if (!hasReadme) gaps.push('no README');
      ...
      // system-design Rule 13 + system-rules Rule 6 (2026-09-06): goal + retention are README keys
      const rd = read(path.join(dir, 'README.md'));
      if (!/^\s*\**goal\**\s*:\s*\S/mi.test(rd)) gaps.push('goal-less');
      if (!/^\s*\**retention\**\s*:\s*\S/mi.test(rd)) gaps.push('no retention');
    }
