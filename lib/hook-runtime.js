...
// M1/M2 turn context (plan §M, 2026-09-06): every row carries turn_id · session_id · qa · phase.
// Loaded fail-open — a missing/broken turn-context must never break a hook.
let turnCtx = null;
try { turnCtx = require(path.join(__dirname, 'turn-context.js')); } catch (_) { turnCtx = null; }
function turnFields(input) {
  try { return turnCtx ? turnCtx.contextFor(input) : {}; } catch (_) { return {}; }
}
function openTurn(input, event) {
  try { if (turnCtx) turnCtx.openTurn(input, event); } catch (_) {}
}
function firstLine(s, n) { return String(s || '').split('\n').map(x => x.trim()).filter(Boolean)[0] ? String(s).split('\n').map(x => x.trim()).filter(Boolean)[0].slice(0, n || 160) : ''; }
...
function wrapMain() {
  ...
  openTurn(input, event);
  const ctx = turnFields(input);
  ...
  let res;
  try {
    res = spawnSync(process.execPath, [target], { input, encoding: 'utf8', timeout: CHILD_TIMEOUT_MS, windowsHide: true, env: { ...process.env, CLAUDE_PROJECT_DIR: REPO_ROOT } });
  } catch (e) {
  ...
  const reason = blocked ? firstLine(stderr || stdout, 160) : (fired ? firstLine(stdout, 160) : '');
  appendTelemetry({
    ts: new Date().toISOString(), hook: hookName, event, mode: 'wrap',
    exit, blocked, fired, dur_ms: Date.now() - t0, ...ctx, ...(reason ? { reason } : {}),
...
function runHook(meta, fn) {
  ...
  const reason = out.blocked ? firstLine(out.blockReason || out.contextOut, 160) : (out.fired ? firstLine(out.contextOut, 160) : '');
  appendTelemetry({
    ts: new Date().toISOString(), hook: meta.name, event: meta.event, mode: 'native',
    exit: out.blocked ? 2 : 0, blocked: !!out.blocked, bypassed: !!out.bypassed, fired: !!out.fired,
    ...(out.bypassToken ? { bypass_token: out.bypassToken } : {}),
    dur_ms: Date.now() - t0, ...ctx, ...(reason ? { reason } : {}), ...(error ? { error } : {}),
  });
  if (out.contextOut) process.stdout.write(out.contextOut);
  if (out.blocked) {
    // Hooks return the block text in either blockReason or contextOut; the harness only
    // shows stderr on exit 2 — losing it surfaces as "hook error: No stderr output".
    const reason = out.blockReason || out.contextOut;
