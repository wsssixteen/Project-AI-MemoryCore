...
// Structured index over etanah-knowledge/<state>/TEST-PERMOHONAN-INDEX.md — the thing Ruri
// always derives when preparing test scenarios. Index lives NEXT TO the source (untracked).
//   build:  node lib/test-data-db.js build [--state <key>]
//   lookup: node lib/test-data-db.js <urusan-code or free text> [--state <key>]   e.g. PPTPB · "no resit PSBS"
// State (2026-09-04, multi-state audit): --state <key> · env ETANAH_STATE · else the registry's reference
// state, ECHOED on line 1 so a default is never silent. Paths come from lib/states.js (system/states.json).
...
const STATE = states.get(STATE_KEY);
if (!STATE) { console.error('test-data-db: unknown state "' + STATE_KEY + '" — node lib/states.js list'); process.exit(2); }
const STATE_NOTE = `state = ${STATE.key}${(_si > 0 || process.env.ETANAH_STATE) ? '' : ' (reference-state default — pass --state <key> for another state)'}`;
const SRC = path.join(states.knowledgeDir(STATE.key), 'TEST-PERMOHONAN-INDEX.md');
